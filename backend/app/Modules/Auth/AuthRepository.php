...
final class AuthRepository
{
    ...
    public function findUser(string $username): ?array
    {
        $statement = $this->db()->prepare(
            'SELECT u.id_usuario, u.usuario, u.nombre, u.hash_contrasena, u.activo,
             ...
             FROM sis_usuarios u
             INNER JOIN sis_roles r ON r.id_rol = u.id_rol
    ...
    public function recentFailureCount(string $username, int $minutes): int
    {
        $statement = $this->db()->prepare(
            'SELECT COUNT(*)
             FROM sis_login_intentos intento
             WHERE intento.usuario = :usuario
               AND intento.exito = 0
               AND intento.creado_en >= :desde
               AND intento.id_intento > COALESCE((
                   SELECT MAX(exito.id_intento) FROM sis_login_intentos exito
    ...
    {
        $statement = $this->db()->prepare(
            'SELECT COUNT(*) FROM sis_login_intentos
    ...
    {
        $statement = $this->db()->prepare(
            'INSERT INTO sis_login_intentos (id_usuario, usuario, ip, user_agent, exito, creado_en)
    ...
    {
        $statement = $this->db()->prepare('UPDATE sis_usuarios SET hash_contrasena = :hash, actualizado_en = NOW() WHERE id_usuario = :id');
        $statement->execute(['hash' => $hash, 'id' => $userId]);
    }

    public function updateLastLogin(int $userId): void
    {
        $this->db()->prepare('UPDATE sis_usuarios SET ultimo_login_en = NOW() WHERE id_usuario = :id')->execute(['id' => $userId]);
    }
    ...
    {
        $this->db()->prepare('UPDATE sis_sesiones SET ultimo_uso_en = NOW() WHERE id_sesion = :id')->execute(['id' => $sessionId]);
    }

    public function deactivateSession(int $sessionId): void
    {
        $this->db()->prepare('UPDATE sis_sesiones SET activa = 0 WHERE id_sesion = :id')->execute(['id' => $sessionId]);
    }

    public function deactivateUserSessions(int $userId): void
    {
        $this->db()->prepare('UPDATE sis_sesiones SET activa = 0 WHERE id_usuario = :usuario')->execute(['usuario' => $userId]);
    }
}
