...
final class PersonasService
{
    ...
    public function create(array $input, array $session, string $correlationId): array
    {
        ...
        try {
            return Connection::transaction(function (PDO $db) use (
                ...
                $personId = $this->repository->insertPerson($normalized['general'], $userId);
                $this->savePersonData($personId, $normalized, $userId);
                $after = $this->repository->find($personId);
                if (!$after) {
                    throw new ApiException('No se pudo recuperar la persona creada.', 'PERSON_CREATE_INCONSISTENT', 500);
                }

                $name = (string)($after['persona']['nombre_exhibicion'] ?? ('ID ' . $personId));
    ...
    public function update(int $personId, array $input, array $session, string $correlationId): array
    {
        ...
        $userId = (int)$session['id_usuario'];
        $normalized = $this->validateAndNormalize($input, $personId, $current['asociado'] !== null);
        ...
        try {
            return Connection::transaction(function (PDO $db) use (
                ...
                $before = $this->repository->find($personId);
                if (!$before) {
                    throw new ApiException('La persona solicitada no existe.', 'PERSON_NOT_FOUND', 404);
                }
                ...
                $after = $this->repository->find($personId);
                if (!$after) {
                    throw new ApiException('No se pudo recuperar la persona actualizada.', 'PERSON_UPDATE_INCONSISTENT', 500);
                }

                $changes = AuditDiff::between($before, $after);
                $name = (string)($after['persona']['nombre_exhibicion'] ?? ('ID ' . $personId));
                $description = (int)($changes['cantidad'] ?? 0) > 0
                    ? "Editó la persona {$name}; se modificaron {$changes['cantidad']} campos."
                    : "Guardó la persona {$name} sin cambios de datos.";
    ...
    public function changeStatus(int $personId, array $input, array $session, string $correlationId): array
    {
        ...
        if ($errors !== []) {
            throw new ApiException('Revisá los datos de la baja.', 'VALIDATION_ERROR', 422, $errors);
        }

        $emptyImpact = ['total' => 0, 'como_titular' => 0, 'como_vinculada' => 0, 'items' => []];
        $confirmLinks = $active
            ? false
            : filter_var($input['confirmar_vinculos'] ?? false, FILTER_VALIDATE_BOOL, FILTER_NULL_ON_FAILURE) === true;
        $userId = (int)$session['id_usuario'];
        ...
        return Connection::transaction(function (PDO $db) use (
            ...
            $linkImpact = $active ? $emptyImpact : $this->repository->activeLinkImpact($personId);
            ...
            $this->repository->setActive($personId, $active, $userId, $reason, $leaveDate);
            $deactivatedLinks = (!$active && $leaveDate !== null)
                ? $this->repository->deactivateActiveLinksForPerson($personId, $leaveDate)
                : 0;
            $after = $this->repository->find($personId);
            if (!$after) {
                throw new ApiException('No se pudo recuperar la persona modificada.', 'PERSON_STATUS_INCONSISTENT', 500);
            }
            ...
            $this->audit->recordForSession(
                ...
                'persona',
                $personId,
                $active ? "Reactivó a {$name}." : "Dio de baja a {$name}. Motivo: {$reason}",
                ...
                [
                    ...
                    'ayudas_vigentes_detectadas' => (int)$aidImpact['total'],
                ],
                'success',
                $correlationId,
                $db
