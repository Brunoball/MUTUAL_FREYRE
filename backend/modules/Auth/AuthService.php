...
final class AuthService
{
    ...
    public function login(array $input, string $correlationId): array
    {
        ...
        if ($this->repository->recentFailureCount($applicationId, $credential, $lockMinutes) >= $maxAttempts) {
            ...
            $this->audit->recordSafely(
                ...
                'credencial',
                $credential,
                ['usuario_intentado' => $credential, 'minutos_bloqueo' => $lockMinutes, 'aplicacion' => $this->applicationCode],
                'blocked',
                $correlationId,
                ['usuario' => $credential, 'nombre' => 'Credencial sin autenticar', 'rol' => 'SIN_AUTENTICAR', 'aplicacion_codigo' => $this->applicationCode],
                "Bloqueó temporalmente el inicio de sesión para la credencial {$credential} por exceso de intentos."
            );
            throw new ApiException(
        ...
        if ($this->repository->recentIpFailureCount($applicationId, $ip, $lockMinutes) >= 20) {
            ...
            $this->audit->recordSafely(
                ...
                'blocked',
                $correlationId,
                ['usuario' => 'origen-' . $ip, 'nombre' => 'Origen sin autenticar', 'rol' => 'SIN_AUTENTICAR', 'aplicacion_codigo' => $this->applicationCode],
                "Bloqueó inicios de sesión desde la IP {$ip} por exceso de intentos."
            );
            throw new ApiException(
        ...
        if (!$valid) {
            ...
            $this->audit->recordSafely(
                ...
                'usuario',
                $userId ?? $credential,
                ['usuario_intentado' => $credential, 'motivo' => 'INVALID_CREDENTIALS', 'aplicacion' => $this->applicationCode],
                'failed',
                $correlationId,
                $user ?: ['usuario' => $credential, 'nombre' => 'Credencial no reconocida', 'rol' => 'SIN_AUTENTICAR', 'aplicacion_codigo' => $this->applicationCode],
                "Falló un inicio de sesión para {$credential}: credenciales inválidas."
            );
            throw new ApiException('Usuario o contraseña incorrectos.', 'INVALID_CREDENTIALS', 401);
        }
        ...
        if (!(bool)$user['activo']) {
            ...
            $this->auditRejectedLogin($user, $credential, 'USER_DISABLED', $correlationId);
        ...
        if (!(bool)$user['acceso_activo']) {
            ...
            $this->auditRejectedLogin($user, $credential, 'APPLICATION_ACCESS_DISABLED', $correlationId);
        ...
        if (
            ...
            $this->auditRejectedLogin($user, $credential, 'USER_TEMPORARILY_BLOCKED', $correlationId);
            throw new ApiException(
        ...
        if ($roles === []) {
            ...
            $this->auditRejectedLogin($user, $credential, 'NO_ROLE_ASSIGNED', $correlationId);
            throw new ApiException(
        ...
        $auditActor = $user;
        $auditActor['roles'] = $roles;
        $auditActor['aplicacion_codigo'] = $this->applicationCode;
        $this->audit->recordSafely(
            $userId,
            'auth',
            'login_exitoso',
            'sesion',
            $sessionId,
            ['aplicacion' => $this->applicationCode, 'expira_en' => $expires->format(DATE_ATOM)],
            'success',
            $correlationId,
            $auditActor,
            "{$user['usuario']} inició sesión correctamente."
        );
    ...
    public function currentFromRequest(Request $request): array
    {
        if ($request->cookie($this->cookieName) === '') {
            return ['autenticado' => false];
        }

        try {
            return $this->current($this->authenticate($request));
        } catch (ApiException $error) {
            if (in_array($error->status, [401, 403, 423], true)) {
                $this->clearSessionCookie();
                return [
                    'autenticado' => false,
                    'motivo' => $error->errorCode,
                ];
            }
            throw $error;
        }
    ...
    public function logout(array $session, string $correlationId): void
    {
        ...
        $this->audit->recordForSessionSafely(
            $session,
            'auth',
            'logout',
            'sesion',
            $sessionId,
            "{$session['usuario']} cerró su sesión.",
            [],
            ['aplicacion' => $this->applicationCode],
            'success',
            $correlationId
    ...
    private function auditRejectedLogin(
        array $user,
        string $credential,
        string $reason,
        ...
        $actor = $user;
        $actor['aplicacion_codigo'] = $this->applicationCode;
        $this->audit->recordSafely(
            (int)$user['id_usuario'],
            'auth',
            'login_rechazado',
            ...
            $correlationId,
            $actor,
            "Se rechazó el inicio de sesión de {$credential}. Motivo: {$reason}."
        );
    }
