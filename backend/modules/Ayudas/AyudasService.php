...
final class AyudasService
{
    ...
    public function create(array $input, array $session, string $correlationId): array
    {
        ...
        try {
            return Connection::transaction(function (PDO $db) use (
                $prepared,
                $userId,
                $session,
                $correlationId
            ): array {
                $created = $this->persistPrepared($prepared, $userId, true, null, 'NUEVA');
                $detail = $this->repository->detail((int)$created['id_ayuda']);
                if (!$detail) {
                    throw new ApiException('No se pudo recuperar la ayuda liquidada.', 'AID_CREATE_INCONSISTENT', 500);
                }

                $number = str_pad((string)$created['numero_ayuda'], 8, '0', STR_PAD_LEFT);
                $this->audit->recordForSession(
                    ...
                    'ayuda_economica',
                    $created['id_ayuda'],
                    "Liquidó la ayuda económica N° {$number} por {$prepared['resumen']['capital_original']} {$prepared['producto']['moneda']}.",
                    AuditDiff::between([], $detail),
                    [
                        'numero_ayuda' => $created['numero_ayuda'],
                        'tipo' => $prepared['producto']['codigo'],
                        'id_asociado' => $prepared['asociado']['id_asociado'],
                        'socio' => $prepared['asociado']['nombre'] ?? null,
                        ...
                        'snapshot_final' => AuditDiff::snapshot($detail),
                    ],
                    'success',
                    $correlationId,
                    $db
    ...
    public function renew(int $aidId, array $input, array $session, string $correlationId): array
    {
        ...
        return Connection::transaction(function (PDO $db) use (
            ...
            $beforeOrigin = $this->repository->detail($aidId);
            if ((string)$origin['tipo'] !== 'B') {
                throw new ApiException('Solo las ayudas tipo B pueden renovarse.', 'AID_NOT_RENEWABLE', 409);
            }
            if ((string)$origin['estado'] !== 'VIGENTE') {
                throw new ApiException('La ayuda ya no se encuentra vigente.', 'AID_NOT_ACTIVE', 409);
            }
            ...
            $renewalInput = [
                ...
                'garantes' => array_map(static fn(array $item): int => (int)$item['id_persona'], $guarantors),
            ...
            $created = $this->persistPrepared($prepared, $userId, false, (int)$origin['id_ayuda'], 'RENOVACION');
            ...
            $afterOrigin = $this->repository->detail($aidId);
            $newDetail = $this->repository->detail((int)$created['id_ayuda']);
            if (!$afterOrigin || !$newDetail) {
                throw new ApiException('No se pudo reconstruir la renovación.', 'AID_RENEWAL_INCONSISTENT', 500);
            }
            ...
            $oldNumber = str_pad((string)$origin['numero_ayuda'], 8, '0', STR_PAD_LEFT);
            $newNumber = str_pad((string)$created['numero_ayuda'], 8, '0', STR_PAD_LEFT);
            $this->audit->recordForSession(
                $session,
                'ayudas',
                'renovar',
                'ayuda_economica',
                $created['id_ayuda'],
                "Renovó la ayuda N° {$oldNumber} y generó la ayuda N° {$newNumber}.",
                AuditDiff::between(
                    ['ayuda_origen' => $beforeOrigin, 'ayuda_nueva' => null],
                    ['ayuda_origen' => $afterOrigin, 'ayuda_nueva' => $newDetail]
                ),
                [
    ...
    public function annul(int $aidId, array $input, array $session, string $correlationId): array
    {
        ...
        return Connection::transaction(function (PDO $db) use (
            ...
            $before = $this->repository->detail($aidId);
            if ((string)$aid['estado'] !== 'VIGENTE') {
                throw new ApiException('Solo puede anularse una ayuda vigente.', 'AID_NOT_ACTIVE', 409);
            }
            ...
            $after = $this->repository->detail($aidId);
            if (!$after) {
                throw new ApiException('No se pudo recuperar la ayuda anulada.', 'AID_ANNUL_INCONSISTENT', 500);
            }

            $number = str_pad((string)$aid['numero_ayuda'], 8, '0', STR_PAD_LEFT);
            $this->audit->recordForSession(
                $session,
                'ayudas',
                'anular',
                'ayuda_economica',
                $aidId,
                "Anuló la ayuda económica N° {$number}. Motivo: {$reason}",
                AuditDiff::between($before, $after),
                [
                    'numero_ayuda' => (int)$aid['numero_ayuda'],
                    'motivo' => $reason,
                    'fecha_anulacion' => $date,
                    'reverso_caja' => (bool)($reversal['reversed'] ?? false),
                    'detalle_reverso' => AuditDiff::snapshot($reversal),
                ],
                'success',
                $correlationId,
                $db
        ...
        try {
            return Connection::transaction(function (PDO $db) use (
                ...
                $this->audit->recordForSession(
                    ...
                    'tasa',
                    $id,
                    "Registró una nueva tasa para el grupo {$group}: TNA {$tna}% desde {$date}.",
                    AuditDiff::between([], $after),
    ...
    public function saveQuote(array $input, array $session, string $correlationId): array
    {
        ...
        return Connection::transaction(function (PDO $db) use (
            ...
            $before = $this->repository->quoteByPeriod($period, true);
            $id = $this->repository->upsertQuote($period, $date, $value, $source, $notes, $userId);
            $after = $this->repository->quoteByPeriod($period, false) ?? [
                'id_cotizacion' => $id,
                'periodo' => $period,
            ...
            $action = $before ? 'actualizar_cotizacion_dolar' : 'crear_cotizacion_dolar';
            $this->audit->recordForSession(
                $session,
                'ayudas',
                $action,
                'cotizacion_dolar',
                $id,
                ($before ? 'Actualizó' : 'Registró') . " la cotización promedio {$period} a {$value} ARS/USD.",
                AuditDiff::between($before ?? [], $after),
                [
                    'periodo' => $period,
                    'fuente' => $source,
