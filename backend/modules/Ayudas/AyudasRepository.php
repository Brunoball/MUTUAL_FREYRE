...
final class AyudasRepository
{
    ...
    public function quoteByPeriod(string $period, bool $forUpdate = false): ?array
    {
        $sql = 'SELECT id_cotizacion, periodo, fecha_referencia, valor_promedio,
                       fuente, observaciones, creado_por, creado_en, actualizado_en
                FROM ae_cotizaciones_dolar
                WHERE periodo = :periodo
                LIMIT 1';
        if ($forUpdate) {
        ...
        return $this->one($sql, ['periodo' => $period]);
    }
