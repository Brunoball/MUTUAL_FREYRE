...
export default function AyudaDetalleModal({
  ...
  const overdue =
    Boolean(Number(aid.vencida || 0)) ||
    (aid.estado === "VIGENTE" &&
      aid.fecha_vencimiento &&
      aid.fecha_vencimiento < localDateValue());
  const renewable =
    canManage &&
    aid.tipo === "B" &&
    aid.estado === "VIGENTE" &&
  ...
  const detailTabs = [
    { value: "summary", label: "Resumen" },
    {
      value: "guarantees",
      label: "Garantes",
      badge: guarantors.length,
    },
    ...(checks.length || installments.length
      ? [
          {
            value: "plan",
            label: "Plan y valores",
            badge: checks.length + installments.length,
          },
        ]
      : []),
    {
      value: "cash",
      label: "Caja e historial",
    ...
    <CrudModal
      footerStart={
        <div className="ayuda-detail-actions">
          ...
          {annullable ? (
            <button
              className="global-button global-button--danger"
              onClick={() => onAnnul?.(detail)}
              type="button"
            >
    ...
    >
      ...
      {aid.tipo_operacion === "RENOVACION" ? (
        <div className="ayuda-alert is-info">
          ...
          <span>
            Esta liquidación es una renovación de la ayuda anterior. No generó
            un segundo desembolso de capital.
          </span>
        </div>
      ) : null}

      <div className="entity-form ayuda-modal__form ayuda-detail-tabs">
        <EntityTabs
          ariaLabel="Secciones del detalle de la ayuda"
          idPrefix="ayuda-detalle-tab"
        ...
        <div className="ayuda-modal__content">
          {activeTab === "summary" ? (
            ...
            >
              <section className="ayuda-detail-section">
                <header>
                  <strong>Condiciones de la liquidación</strong>
                </header>
                <div className="ayuda-detail-grid">
                  <Datum label="Solicitud">
                    N° {aidNumber(aid.numero_solicitud)}
                  </Datum>
                  <Datum label="Fecha solicitud">
                    {formatDate(aid.fecha_solicitud)}
                  </Datum>
                  <Datum label="Fecha liquidación">
                    {formatDate(aid.fecha_liquidacion)}
                  </Datum>
                  <Datum label="Vencimiento final">
                    {formatDate(aid.fecha_vencimiento)}
                  </Datum>
                  <Datum label="Sistema">
                    {text(aid.sistema_amortizacion)}
                  </Datum>
                  <Datum label="Periodicidad">{text(aid.periodicidad)}</Datum>
                  <Datum label="Plazo">
                    {aid.plazo_cantidad || "—"} {text(aid.plazo_unidad, "")}
                  </Datum>
                  <Datum label="Garantía">{text(aid.tipo_garantia)}</Datum>
                  <Datum label="Base de cálculo">
                    {aid.base_dias || 365} días
                  </Datum>
                ...
                {aid.observaciones ? (
                  <p className="ayuda-detail-note">
                    <b>Observaciones:</b> {aid.observaciones}
                  </p>
                ) : null}
              </section>
              ...
              <section className="ayuda-detail-section">
                <header>
                  <strong>Importes y tasas</strong>
                </header>
                <div className="ayuda-summary-grid ayuda-summary-grid--detail">
                  <div>
                    <span>Capital</span>
                    <strong>
                      {formatMoney(aid.capital_original, currency)}
                    </strong>
                  </div>
                  {aid.tipo === "I" ? (
                    <>
                      <div>
                        ...
                        <strong>
                          {formatMoney(aid.cotizacion_dolar, "ARS")}
                        </strong>
                      </div>
                      <div>
                        <span>Equivalente liquidado</span>
                        <strong>
                          {formatMoney(aid.capital_equivalente_ars, "ARS")}
                        </strong>
                      </div>
                    </>
                  ) : null}
                  <div>
                    ...
                    <strong>
                      {formatMoney(aid.devengamiento_total, currency)}
                    </strong>
                  </div>
                  <div>
                    <span>Gastos administrativos</span>
                    <strong>
                      {formatMoney(aid.gastos_administrativos, currency)}
                    </strong>
                  </div>
                  <div>
                    <span>Otros / recupero</span>
                    <strong>
                      {formatMoney(
                        Number(aid.otros_gastos || 0) +
                          Number(aid.recupero_gastos || 0),
                        currency,
                      )}
                    </strong>
                  </div>
                  <div>
                    <span>Sellado / seguro</span>
                    <strong>
                      {formatMoney(
                        Number(aid.sellado || 0) + Number(aid.seguro || 0),
                        currency,
                      )}
                    </strong>
                  </div>
                  <div>
                    ...
                    <strong>
                      {formatMoney(aid.total_a_devolver, currency)}
                    </strong>
                  </div>
                  <div>
                    <span>Acreditado en caja común</span>
                    <strong>
                      {formatMoney(aid.importe_acreditado_ars, "ARS")}
                    </strong>
                  </div>
                  <div>
                    <span>TEM / TNA</span>
                    <strong>
                      {formatRate(aid.tem)} / {formatRate(aid.tna)}
                    </strong>
                  </div>
                  <div>
                    <span>TEA / CFT</span>
                    <strong>
                      {formatRate(aid.tea)} / {formatRate(aid.cft)}
                    </strong>
                  </div>
                </div>
              </section>
          ...
          {activeTab === "guarantees" ? (
            <EntityFormPanel
              eyebrow="Respaldo de la operación"
              idPrefix="ayuda-detalle-tab"
              tabValue="guarantees"
              tag={`${guarantors.length} registrado${guarantors.length === 1 ? "" : "s"}`}
              title="Garantes"
            >
              <section className="ayuda-detail-section">
                <header>
                ...
                {guarantors.length ? (
                  ...
                  <p className="ayuda-empty-inline">
                    No se registraron garantes.
                  </p>
                )}
              </section>
            </EntityFormPanel>
          ...
          {activeTab === "plan" ? (
            <EntityFormPanel
              eyebrow="Cronograma y documentación"
              idPrefix="ayuda-detalle-tab"
              tabValue="plan"
              title="Plan de pagos y valores"
            >
              {checks.length ? (
                <section className="ayuda-detail-section">
                  <header>
                      ...
                      <tbody>
                        {checks.map((item) => (
                          <tr key={item.id_cheque}>
                            ...
                            <td>
                              {Number(item.electronico) ? "ECHEQ" : "FÍSICO"}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <table className="ayuda-data-table">
                      ...
                      <tbody>
                        {installments.map((item) => {
                          const costs =
                            Number(item.gastos_administrativos || 0) +
                          ...
                          return (
                            <tr key={item.id_cuota || item.numero_cuota}>
                              ...
                              <td>
                                {formatMoney(item.saldo_inicial, currency)}
                              </td>
                              <td>
                                {formatMoney(
                              ...
                              <td>
                                {formatMoney(item.devengamiento, currency)}
                              </td>
                              <td>{formatMoney(costs, currency)}</td>
                              <td>{formatMoney(item.seguro, currency)}</td>
                              <td>
                                <b>
                                  {formatMoney(item.importe_cuota, currency)}
                                </b>
                              </td>
                              <td>
                                <span
                                  className={`global-chip ${item.estado === "PAGADA" ? "is-success" : item.estado === "ANULADA" ? "is-danger" : "is-warning"}`}
                                >
                                  {item.estado}
                                </span>
                              </td>
          ...
          {activeTab === "cash" ? (
            <EntityFormPanel
              eyebrow="Movimientos relacionados"
              idPrefix="ayuda-detalle-tab"
              tabValue="cash"
              tag={`${movements.length} movimiento${movements.length === 1 ? "" : "s"}`}
              title="Caja de ahorro e historial"
            >
              <section className="ayuda-detail-section">
                <header>
                  <strong>Caja de ahorro común</strong>
                </header>
                <div className="ayuda-detail-grid">
                  <Datum label="Saldo actual" strong>
                    {formatMoney(aid.saldo_caja_ahorro_comun, "ARS")}
                  </Datum>
                  <Datum label="Medio de desembolso">
                    {text(aid.medio_desembolso)}
                  </Datum>
                  <Datum label="Mutuo">
                    N° {aidNumber(detail?.mutuo?.numero_mutuo)}
                  </Datum>
                  <Datum label="Estado del mutuo">
                    {text(detail?.mutuo?.estado)}
                  </Datum>
                </div>
                {movements.length ? (
                  <div className="ayuda-movement-list">
                    {movements.map((item) => (
                      <div key={item.id_movimiento}>
                        ...
                        <small>
                          Saldo: {formatMoney(item.saldo_posterior, "ARS")}
                        </small>
                      </div>
                    ))}
                  </div>
                  ...
                  <p className="ayuda-empty-inline">
                    Esta ayuda no registró movimiento de desembolso, por ejemplo
                    por tratarse de una renovación.
                  </p>
                )}
              </section>
              ...
              {detail?.renovacion ? (
                <div className="ayuda-alert is-info">
                  <GlobalIcon name="refresh" size={18} />
                  <span>
                    Renovada el {formatDate(detail.renovacion.fecha_renovacion)}{" "}
                    como ayuda N°{" "}
                    {aidNumber(detail.renovacion.numero_ayuda_nueva)}. Intereses
                    cobrados:{" "}
                    {formatMoney(detail.renovacion.intereses_cobrados, "ARS")}.
                  </span>
                </div>
              ) : null}
              ...
              {aid.motivo_anulacion ? (
                <div className="ayuda-alert is-error">
                  ...
                  <span>
                    <b>Motivo de anulación:</b> {aid.motivo_anulacion}
                  </span>
                </div>
              ) : null}
            </EntityFormPanel>
